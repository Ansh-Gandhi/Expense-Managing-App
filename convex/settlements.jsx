...
export const getSettlementData = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.entityType === "user") {
            ...
            const myExpenses = await ctx.db.query("expenses").withIndex("by_user_and_group", (q) => 
                q.eq("paidByUserId", me._id).eq("groupId", undefined)).collect();

            const otherUserExpenses = await ctx.db.query("expenses").withIndex("by_user_and_group", (q) =>
                q.eq("paidByUserId", other._id).eq("groupId", undefined)).collect();
            ...
            for (const e of expenses) {
                const involvesMe = e.paidByUserId === me._id || e.splits.some((s) => s.userId === me._id);
                const involvesThem = e.paidByUserId === other._id || e.splits.some((s) => s.userId === other._id);
            ...
            const mySettlements = await ctx.db.query("settlements").withIndex("by_user_and_group", (q) =>
                q.eq("paidByUserId", me._id).eq("groupId", undefined)).collect();

            const otherUserSettlements = await ctx.db.query("settlements").withIndex("by_user_and_group", (q) =>
                q.eq("paidByUserId", other._id).eq("groupId", undefined)).collect();
            ...
            for (const e of expenses) {
                ...
                } else if (balances[e.paidByUserId]) {
                    const split = e.splits.find((s) => s.userId === me._id && !s.paid);
                    if (split) {
                        balances[e.paidByUserId].owing += split.amount;
                    }
                }
            }

            const settlements = await ctx.db.query("settlements").filter((q) => q.eq(q.field("groupId"), group._id)).collect();

            for (const s of settlements) {
                if (s.paidByUserId === me._id && balances[s.receivedByUserId]) {
                    balances[s.receivedByUserId].owing = Math.max(
                        0,
                        balances[s.receivedByUserId].owing - s.amount
                    );
                }
                if (s.receivedByUserId === me._id && balances[s.paidByUserId]) {
                    balances[s.paidByUserId].owed = Math.max(0, balances[s.paidByUserId].owed - s.amount);
                }
            }
